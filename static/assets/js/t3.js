...
window.addEventListener("load", () => {
  ...
  if (form && input) {
    form.addEventListener("submit", async event => {
      ...
      // Use saved search engine (same as index page) for non-URLs
  ...
  function processUrl(url) {
    ...
    activeIframe.src = `${getProxyPrefix()}${encoded}`;
...
document.addEventListener("DOMContentLoaded", event => {
  ...
  function createNewTab() {
    ...
    newIframe.sandbox =
      "allow-same-origin allow-scripts allow-forms allow-pointer-lock allow-modals allow-orientation-lock allow-presentation allow-storage-access-by-user-activation allow-popups allow-popups-to-escape-sandbox allow-top-navigation-by-user-activation";
    ...
    newIframe.addEventListener("load", () => {
      ...
      newIframe.contentWindow.open = url => {
        ...
        sessionStorage.setItem("URL", `${prefix}${encoded}`);
        createNewTab();
        return null;
      };
    ...
    if (tabCounter === 0 || tabCounter === 1) {
      if (goUrl !== null) {
        if (goUrl.includes("/e/")) {
          ...
          const prefix = getProxyPrefix();
          newIframe.src = `${window.location.origin}${prefix}${goUrl}`;
        }
      } else {
        newIframe.src = "/";
      }
